...
export default async function AvailabilityPage() {
  ...
  // Independent pilots have no Skywings availability planning. Fetched
  // separately so the page still loads if migration 023 hasn't run yet.
  const { data: typeRow } = await supabase
    .from('pilots').select('pilot_type').eq('id', user.id).maybeSingle();
  if ((typeRow as { pilot_type?: string } | null)?.pilot_type === 'independent') redirect('/home');
    ...
    <div className="p-4 space-y-4 max-w-xl mx-auto">
      ...
      <div data-tour="avail-calendar">
        <AvailabilityCalendar
          ...
          googleConnected={!!pilot.google_refresh_token && (pilot.google_enabled ?? true)}
        />
      </div>
      {(pilot.google_enabled ?? true) && (
